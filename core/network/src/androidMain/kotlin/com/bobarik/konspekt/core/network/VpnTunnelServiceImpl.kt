...
import com.bobarik.konspekt.core.network.models.MessageData
...
class VpnTunnelServiceImpl : VpnTunnelService, VpnService() {
  ...
  override fun onBind(intent: Intent): IBinder? {
    Log.info("Binding VPN service: $intent")
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    Log.info("Starting VPN service: $intent")
    if (intent != null) {
  ...
  private fun startTunnel(
    ...
    Log.info("Starting tunnel ${config.id} for server ${config.name}")
    ...
    val alreadyRunning = this.tunnelConfig != null && this.tunFd != null
    if (alreadyRunning) {
    ...
    val clientResult: NewClientResult = Outline.newClient(config.transportConfig)
    if (clientResult.error != null) {
      Log.log(Level.WARNING, "Failed to create Outline Client", clientResult.error)
      tearDownActiveTunnel()
      throw error("Failed to create Outline Client")
    }
    val client: outline.Client? = clientResult.client
    ...
    if (isAutoStart) {
      ...
      try {
        ...
        val connResult: TCPAndUDPConnectivityResult = Outline.checkTCPAndUDPConnectivity(client)
        Log.info("Go connectivity check result: $connResult")
    ...
    val result: ConnectOutlineTunnelResult = Tun2socks.connectOutlineTunnel(tunFd?.fd?.toLong() ?: 0L, client, remoteUdpForwardingEnabled)
  ...
  private fun broadcastVpnConnectivityChange(status: TunnelStatus) {
    if (tunnelConfig == null) {
      Log.warning("Tunnel disconnected, not sending VPN connectivity broadcast")
      return
    }
    val statusChange = Intent(StatusBroadcastKey)
    statusChange.addCategory(packageName)
    statusChange.setPackage(packageName)
    statusChange.putExtra(MessageData.Payload.value, status.value)
    statusChange.putExtra(MessageData.TunnelId.value, tunnelConfig!!.id) // TODO: extract nullables into its own config...
  ...
  companion object {
    private val Log: Logger = Logger.getLogger(VpnTunnelServiceImpl::class.java.getName())
    ...
    private val DnsResolverIPAddresses: Array<String> = arrayOf<String>(
    ...
    const val StatusBroadcastKey: String = "onStatusChange"
  }
